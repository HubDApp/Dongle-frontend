/**
 * English (en) messages
 * This is the default/source locale for the application
 */

export const en = {
  common: {
    loading: "Loading...",
    error: "Error",
    success: "Success",
    cancel: "Cancel",
    confirm: "Confirm",
    save: "Save",
    delete: "Delete",
    edit: "Edit",
    submit: "Submit",
    close: "Close",
    back: "Back",
    next: "Next",
    search: "Search",
    filter: "Filter",
    sort: "Sort",
    clear: "Clear",
    clearAll: "Clear all",
    tryAgain: "Try again",
    learnMore: "Learn More",
    viewAll: "View all",
    loadMore: "Load More",
    home: "Home",
  },

  nav: {
    discover: "Discover",
    reviews: "Reviews",
    verify: "Verify",
    submitProject: "Submit Project",
    profile: "Profile",
    admin: "Admin",
    documentation: "Documentation",
    analytics: "Analytics",
    openMenu: "Open menu",
    closeMenu: "Close menu",
  },

  wallet: {
    connect: "Connect Wallet",
    connecting: "Connecting...",
    disconnect: "Disconnect",
    connected: "Connected",
    wrongNetwork: "Wrong network detected.",
    networkMismatch: "Your Freighter wallet is currently on {currentNetwork}. This app requires {expectedNetwork}. Open Freighter, go to Settings → Network, and switch to {expectedNetwork} before submitting any transactions.",
    expectedNetwork: "Expected: {network}",
  },

  hero: {
    badge: "Now live on Stellar Testnet",
    title: "The Trust Layer",
    titleHighlight: "for Web3 Apps",
    subtitle: "Dongle is the decentralized app store that brings transparency to dApps on Stellar. Discover, review, and verify protocols with on-chain trust.",
    exploreApps: "Explore Apps",
    poweredBy: "POWERED BY",
    verifiedOnChain: "Verified On-Chain",
    status: "Status",
    reviewsCount: "{count} Reviews",
  },

  features: {
    sectionTitle: "Why Dongle?",
    sectionSubtitle: "We're building the infrastructure for a more trustworthy decentralized ecosystem.",
    onChainVerification: {
      title: "On-Chain Verification",
      description: "Every app listing and review is stored directly on the Stellar blockchain, ensuring data integrity and censorship resistance.",
    },
    communityReviews: {
      title: "Community Reviews",
      description: "Real users provide verified feedback. Reviewers earn reputation based on the accuracy and depth of their contributions.",
    },
    developerFocused: {
      title: "Developer Focused",
      description: "Simple integration for developers to list their Stellar dApps and reach a community that values trust and transparency.",
    },
  },

  cta: {
    title: "Ready to list your application?",
    subtitle: "Join the growing ecosystem of transparent dApps on Stellar. Get verified and build trust with your users today.",
  },

  discover: {
    title: "DISCOVER",
    subtitle: "Explore the ecosystem of decentralized applications, infrastructure, and tools built on Stellar and Soroban.",
    searchPlaceholder: "Search projects by name or description...",
    filterByTags: "Filter by Tags",
    addTagsPlaceholder: "Add tags to filter...",
    allStatus: "All Status",
    verified: "Verified",
    pending: "Pending",
    unverified: "Unverified",
    rejected: "Rejected",
    highestRated: "Highest Rated",
    mostPopular: "Most Popular",
    newest: "Newest",
    noProjectsFound: "No projects found",
    noProjectsDescription: "Try adjusting your search or filters to find what you're looking for.",
    clearFilters: "Clear Filters",
    loadingProjects: "Loading projects...",
    loadMoreProjects: "Load More Projects",
  },

  projectCard: {
    reviews: "{count} reviews",
    addedAt: "Added {date}",
    saveProject: "Save {name}",
    removeFromSaved: "Remove {name} from saved projects",
    addToComparison: "Add {name} to comparison",
    removeFromComparison: "Remove {name} from comparison",
    maxProjectsReached: "Cannot add {name}: maximum 4 projects",
  },

  projectForm: {
    title: {
      create: "Submit New Project",
      edit: "Edit Project",
    },
    basicInfo: "Basic Information",
    projectName: "Project Name",
    projectNamePlaceholder: "My Awesome DApp",
    description: "Description",
    descriptionPlaceholder: "Tell us about your project...",
    category: "Primary Category",
    tags: "Tags",
    tagsPlaceholder: "Add relevant tags...",
    websiteUrl: "Website URL",
    websiteUrlPlaceholder: "https://...",
    githubUrl: "Repository URL (Optional)",
    githubUrlPlaceholder: "https://github.com/owner/repo",
    logoUrl: "Logo URL (Optional)",
    logoUrlPlaceholder: "https://...",
    docsUrl: "Documentation URL (Optional)",
    docsUrlPlaceholder: "https://docs...",
    auditReportUrl: "Audit Report URL (Optional)",
    bugBountyUrl: "Bug Bounty URL (Optional)",
    submitButton: {
      create: "Submit Registration",
      edit: "Update Project",
      processing: "Processing Transaction...",
    },
    disclaimer: {
      create: "By submitting, you agree to have your project details stored on the Stellar network. A small transaction fee will be required for on-chain registration.",
      edit: "By updating, you agree to have your project details updated on the Stellar network.",
    },
    duplicateWarning: {
      title: "Possible Duplicate Detected",
      description: "We found existing projects that look very similar to yours:",
      confirmLabel: "Continue Anyway",
    },
    discardDraft: {
      title: "Discard Draft",
      description: "Are you sure you want to discard this draft? All unsaved changes will be lost.",
      confirmLabel: "Discard Draft",
      cancelLabel: "Keep Draft",
    },
  },

  reviews: {
    title: "YOUR REVIEWS",
    subtitle: "See what the community is saying about Stellar dApps.",
    addReview: "Add Review",
    editReview: "Edit Review",
    yourReviews: "Your Reviews",
    noReviews: "No reviews yet. Start reviewing projects!",
    rating: "Rating",
    comment: "Comment",
    commentPlaceholder: "Share your experience with this project...",
    minChars: "Min: {count} chars",
    postReview: "Post Review",
    updateReview: "Update Review",
    rateStars: "Rate {count} star{plural}",
    devOnlyBadge: "DEV-ONLY",
  },

  verification: {
    title: "Verification Status",
    searchPlaceholder: "Enter Project ID to check status",
    statuses: {
      none: {
        title: "Not Found",
        description: "No verification request found for this project.",
      },
      pending: {
        title: "Pending Review",
        description: "The verification request is currently being reviewed by the community.",
      },
      verified: {
        title: "Verified",
        description: "This project has been verified and is considered trustworthy.",
      },
      rejected: {
        title: "Rejected",
        description: "The verification request for this project was rejected.",
      },
    },
    projectId: "ID: {id}",
  },

  profile: {
    title: "YOUR PROFILE",
    subtitle: "Manage your account and view your activity on Dongle.",
    accountSummary: "Account Summary",
    walletAddress: "Wallet Address",
    network: "Network",
    networkInfo: "Connected to {network}",
    networkDisclaimer: "Dongle requires Stellar {expectedNetwork} (Soroban testnet). Before signing, confirm Freighter is on this network via Settings → Network. Wrong-network transactions are blocked automatically.",
    balances: "Balances",
    activityStats: "Activity Stats",
    avgRating: "Avg Rating",
    submitted: "Submitted",
    quickActions: "Quick Actions",
    browseProjects: "Browse Projects",
    viewAllReviews: "View All Reviews",
    savedProjects: "Saved Projects",
    noSavedProjects: "No saved projects yet. Bookmark projects to revisit them later.",
    submittedProjects: "Submitted Projects",
    noSubmittedProjects: "No submitted projects yet.",
    submitFirstProject: "Submit Your First Project",
    submittedAt: "Submitted {date}",
    rejectionReason: "Rejection Reason:",
    clearHistory: "Clear viewing history",
    clearHistoryConfirm: "This will permanently remove your recently viewed projects. This action cannot be undone.",
    clearHistoryButton: "Clear History",
    discoverProjects: "Discover Projects",
  },

  comparison: {
    title: "Compare Projects",
    count: "Compare Projects ({current}/{max})",
    compareNow: "Compare Now",
    selectAtLeast: "Select at least 2 projects to compare",
  },

  transaction: {
    phases: {
      preparing: "Preparing",
      signing: "Awaiting signature",
      submitting: "Submitting",
      confirming: "Confirming",
    },
    status: {
      idle: "Ready",
      inProgress: "Transaction in progress",
      success: "Transaction complete",
      failure: "Transaction failed",
    },
    retry: "Retry transaction",
  },

  errors: {
    somethingWrong: "Something went wrong",
    somethingWrongIn: "Something went wrong in {section}",
    unexpectedError: "An unexpected error occurred. You can try again or go back home.",
    pageNotFound: "Page Not Found",
    pageNotFoundDescription: "The page you're looking for doesn't exist or has been moved.",
  },

  footer: {
    tagline: "The decentralized app store for Stellar. Discovery, reviews, and verification powered by on-chain transparency.",
    platform: "Platform",
    resources: "Resources",
    github: "GitHub",
    privacyPolicy: "Privacy Policy",
    termsOfService: "Terms of Service",
    copyright: "© {year} Dongle Protocol. All rights reserved.",
  },

  meta: {
    title: "Dongle - Your Onchain App Store",
    description: "The decentralized app store for Stellar. Discovery, reviews, and verification powered by on-chain transparency.",
  },

  legal: {
    lastUpdated: "Last updated: {date}",
  },

  language: {
    label: "Language",
    select: "Select language",
    en: "English",
    es: "Español",
    pt: "Português",
    changed: "Language updated",
    loadError: "Could not load translations. Showing default language.",
    invalid: "Unsupported language. Falling back to English.",
  },

  notifications: {
    title: "Notifications",
    empty: "No notifications yet",
    markAllRead: "Mark all as read",
    markRead: "Mark as read",
    unread: "{count} unread",
    open: "Open notifications",
    close: "Close notifications",
    viewProject: "View project",
    viewReview: "View review",
    live: "Live",
    connecting: "Connecting…",
    reconnecting: "Reconnecting…",
    offline: "Offline — retrying",
    toastDismissed: "Notification dismissed",
    types: {
      project_verified: "Project verified",
      project_rejected: "Project rejected",
      review_approved: "Review approved",
      review_rejected: "Review rejected",
      verification_evidence_requested: "Verification evidence requested",
      claim_received: "Claim received",
      claim_approved: "Claim approved",
      claim_rejected: "Claim rejected",
      unknown: "Update",
    },
    messages: {
      project_verified: "{projectName} has been verified.",
      project_rejected: "{projectName} was rejected.",
      review_approved: "Your review of {projectName} was approved.",
      review_rejected: "Your review of {projectName} was rejected.",
      verification_evidence_requested: "Additional evidence is requested for {projectName}.",
      claim_received: "Your claim for {projectName} was submitted.",
      claim_approved: "Your claim for {projectName} was approved.",
      claim_rejected: "Your claim for {projectName} was rejected.",
    },
    errors: {
      streamFailed: "Could not connect to live notifications.",
      malformed: "Ignored an invalid notification payload.",
      unauthorized: "Sign in to receive notifications.",
    },
  },

  analytics: {
    title: "Trends & Analytics",
    subtitle: "Platform metrics from project and review data.",
    nav: "Analytics",
    trending: "Trending projects",
    trendingHint: "Ranked by reviews per week in the selected range.",
    topCategories: "Top categories",
    newProjectsWeek: "New projects / week",
    timeSeries: "Activity over time",
    totalProjects: "Total projects",
    totalReviews: "Total reviews",
    verificationRate: "Verification rate",
    averageRating: "Average rating",
    medianReviews: "Median review count",
    approvalRate: "Verification approval rate",
    filters: "Filters",
    range: "Date range",
    range7d: "7 days",
    range30d: "30 days",
    range90d: "90 days",
    rangeAll: "All time",
    category: "Category",
    allCategories: "All categories",
    verificationStatus: "Verification status",
    allStatuses: "All statuses",
    exportCsv: "Export CSV",
    exporting: "Exporting…",
    exportFailed: "Could not export CSV.",
    lastUpdated: "Last updated {date} UTC",
    stale: "Showing cached analytics. A refresh is scheduled for 00:00 UTC.",
    empty: "No analytics data for this filter.",
    loading: "Loading analytics…",
    loadFailed: "Could not load analytics.",
    retry: "Retry",
    rpcFailed: "Soroban RPC aggregation failed; catalog data is shown instead.",
    chartProjects: "Projects",
    chartReviews: "Reviews",
    chartVerification: "Verification rate",
    reviewsPerWeek: "{count} reviews/week",
    formSubmissions: {
      title: "Form Submission Analytics",
      subtitle: "Track form engagement, errors, and completion rates.",
      totalSubmissions: "Total Submissions",
      errorRate: "Error Rate",
      abandonmentRate: "Abandonment Rate",
      fieldCompletion: "Avg Field Completion",
      submissionCountByPeriod: "Submission Count by Period",
      submissions: "Submissions",
      fieldCompletionRates: "Field Completion Rates",
    },
    backupRecovery: {
      title: "Form Backup & Recovery",
      subtitle: "Manage automatic form backups and restore data.",
      backupEnabled: "Automatic backups enabled",
      backupDisabled: "Automatic backups disabled",
      backupInterval: "Backup interval",
      maxBackups: "Max backups to keep",
      retentionPeriod: "Retention period",
      createBackup: "Create backup now",
      restoreBackup: "Restore from backup",
      restoreFrom: "Restore from backup #{id}",
      restoreConfirm: "Are you sure you want to restore this backup? Current form data will be replaced.",
      restoreSuccess: "Form data restored from backup.",
      restoreFailed: "Failed to restore backup.",
      deleteBackup: "Delete backup",
      deleteConfirm: "Are you sure you want to delete this backup?",
      deleteSuccess: "Backup deleted.",
      deleteFailed: "Failed to delete backup.",
      noBackups: "No backups available.",
      backupCreated: "Backup created.",
      backupFailed: "Backup failed.",
      retentionCleaned: "Clean expired backups",
      lastBackup: "Last backup: {date}",
      backupCount: "{count} backup(s)",
      configTitle: "Backup Configuration",
      configSaved: "Backup configuration saved.",
      configReset: "Reset to defaults",
      justNow: "just now",
      minutesAgo: "{count} min ago",
      hoursAgo: "{count} hr ago",
      daysAgo: "{count} day(s) ago",
    },
    formArchive: {
      title: "Form Data Archive",
      subtitle: "Manage archived form submissions for compliance.",
      archivePolicy: "Archive Policy",
      archiveAfter: "Archive backups older than",
      maxArchivedBackups: "Max archived backups to keep",
      maxArchivedAge: "Max archived age",
      automatedArchival: "Automated archival",
      automatedArchivalEnabled: "Automated archival enabled",
      automatedArchivalDisabled: "Automated archival disabled",
      archivedBackups: "Archived Backups",
      searchPlaceholder: "Search archived backups...",
      searchResults: "{count} result(s) found",
      noArchivedBackups: "No archived backups.",
      archivedAt: "Archived {date}",
      archivedBy: "Archived by {reason}",
      restoreFromArchive: "Restore from archive",
      restoreFromArchiveConfirm: "Are you sure you want to restore this archived backup? Current form data will be replaced.",
      restoreFromArchiveSuccess: "Form data restored from archive.",
      restoreFromArchiveFailed: "Failed to restore archived backup.",
      deleteArchivedBackup: "Delete archived backup",
      deleteArchivedConfirm: "Are you sure you want to delete this archived backup? This action cannot be undone.",
      deleteArchivedSuccess: "Archived backup deleted.",
      deleteArchivedFailed: "Failed to delete archived backup.",
      cleanExpiredArchived: "Clean expired archived backups",
      archiveRetentionCleaned: "Archived retention cleaned: {removed} removed, {remaining} remaining.",
      lastArchived: "Last archived: {date}",
      archivedCount: "{count} archived backup(s)",
      configTitle: "Archive Configuration",
      configSaved: "Archive configuration saved.",
      configReset: "Reset to defaults",
      archiveNow: "Archive old backups now",
      archivingComplete: "Archiving complete: {count} backup(s) archived.",
      archivalRunning: "Running automated archival...",
      justNow: "just now",
      minutesAgo: "{count} min ago",
      hoursAgo: "{count} hr ago",
      daysAgo: "{count} day(s) ago",
    },
  },

  auth: {
    signIn: "Sign in",
    signOut: "Sign out",
    signInWithGoogle: "Sign in with Google",
    signInWithGitHub: "Sign in with GitHub",
    connectWallet: "Connect Wallet",
    signedInAs: "Signed in as {name}",
    providerGoogle: "Google",
    providerGitHub: "GitHub",
    providerWallet: "Wallet",
    readOnlyBanner: "You are browsing in read-only mode. Connect a Stellar wallet to publish on-chain.",
    walletRequiredTitle: "Wallet signature required",
    walletRequiredBody: "Signing in with Google or GitHub lets you browse. Publishing reviews or listings still requires a Freighter wallet signature.",
    walletRequiredCta: "Connect Freighter",
    oauthDenied: "Sign-in was cancelled.",
    oauthError: "Sign-in failed. Please try again.",
    oauthInvalidState: "Sign-in could not be verified. Please try again.",
    sessionExpired: "Your session expired. Please sign in again.",
    duplicateAccount: "An account already exists for this identity.",
    missingProfile: "The provider did not return enough profile data to sign in.",
    linkWallet: "Link wallet",
    walletLinked: "Wallet linked",
    account: "Account",
  },

  gdpr: {
    consent: {
      title: "Data Privacy & Consent",
      description: "We process your form data in accordance with our privacy practices. You have the right to export, delete, or control your data at any time.",
      label: "I agree to the processing of my form data as described in the {privacyPolicy}.",
      privacyPolicyLink: "Privacy Policy",
      required: "Consent is required to submit this form.",
      purposes: {
        form_submission: "Process form submissions",
        data_processing: "Store and process data for service improvement",
        analytics: "Anonymous analytics to improve user experience",
        backup: "Create encrypted backups of your form data",
      },
    },
    export: {
      button: "Export My Data",
      exporting: "Preparing export…",
      exported: "Export ready — check your downloads",
      description: "Download a copy of all data we hold about you.",
    },
    deletion: {
      button: "Delete My Data",
      dialogTitle: "Delete your data?",
      dialogDescription: "This will permanently remove all your form data, backups, and consent records. This action cannot be undone.",
      confirm: "Yes, delete my data",
      cancel: "Cancel",
      success: "Your data has been deleted successfully.",
      description: "Request permanent deletion of all your form data.",
    },
    retention: {
      title: "Data Retention Policy",
      description: "We retain your form data only as long as necessary. Backups are automatically cleaned after {maxBackupAge} days. Archived data is removed after {maxArchiveAge} days.",
      maxBackupAge: "30",
      maxArchiveAge: "90",
      consentRetention: "Consent records are retained for {consentRetention} days.",
    },
  },

  footer: {
    tagline: "The trust layer for Web3 applications.",
    platform: "Platform",
    resources: "Resources",
    copyright: "© {year} Dongle. All rights reserved.",
    privacyPolicy: "Privacy Policy",
    termsOfService: "Terms of Service",
    github: "GitHub",
  },
} as const;

export type Messages = typeof en;
